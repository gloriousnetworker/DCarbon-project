...
export default function ResidentialWelcomeBackCard() {
  const [loading, setLoading] = useState(false);
  const router = useRouter();

  const handleResidentialRegistration = () => {
    setLoading(true);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="w-full text-center mb-8">
          <p className="font-sfpro text-[16px] leading-[140%] tracking-[-0.05em] text-[#1E1E1E] mb-4">
            Thank you for joining DCarbon as a Residential user! Now that you've completed your initial registration,
            let's continue with your residential setup.
          </p>
          
          <p className="font-sfpro text-[14px] leading-[140%] tracking-[-0.05em] text-[#626060] mb-6">
            As a <span className="font-semibold">residence owner</span>, you'll be able to track your energy usage,
            solar production, and carbon offset contributions.
          </p>
          
          <p className="font-sfpro text-[14px] leading-[140%] tracking-[-0.05em] text-[#1E1E1E] font-medium">
            Click the button below to complete your residential registration:
          </p>
        </div>

        {/* Residential Registration Button */}
        <div className="w-full flex justify-center">
          <button
            type="button"
            onClick={handleResidentialRegistration}
          ...
          >
            Continue Residential Registration
          </button>
        </div>
      </div>
    </div>
