...
export default function UserSalesStatement() {
  ...
  useEffect(() => {
    ...
    if (storedUserId && authToken) {
      fetchUserSalesStatement(storedUserId);
    } else {
      setError("Missing userId or authToken in localStorage");
      setLoading(false);
    }
  }, []);
  ...
  const fetchUserSalesStatement = async (userId) => {
    try {
      ...
      console.log("Fetching sales statement for userId:", userId);
      console.log("API URL:", `https://services.dcarbon.solutions/api/rec/sale-statement/${userId}`);

      const response = await fetch(
        `https://services.dcarbon.solutions/api/rec/sale-statement/${userId}`,
        {
          method: 'GET',
          headers: {
      ...
      console.log("Sales statement data:", data);
      ...
      console.error("Error fetching user sales statement:", err);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p className="text-gray-800">{statementData.userInfo.companyName}</p>
            </div>
            <div>
      ...
      {statementData?.overview && (
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            <p className="text-xl font-bold text-green-800">{formatNumber(statementData.overview.totalRecsGenerated)}</p>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <h5 className="text-sm font-medium text-purple-600">Total RECs Sold</h5>
            <p className="text-xl font-bold text-purple-800">{formatNumber(statementData.overview.totalRecsSold)}</p>
          </div>
          <div className="bg-orange-50 p-4 rounded-lg">
            <h5 className="text-sm font-medium text-orange-600">Current REC Balance</h5>
            <p className="text-xl font-bold text-orange-800">{formatNumber(statementData.overview.currentRecBalance)}</p>
          </div>
        </div>
      )}
            ...
            {statementData?.sales && statementData.sales.length > 0 ? (
                ...
                <tr key={sale.saleId}>
                  ...
                  <td className="py-3 px-3">
                    <span className={`px-2 py-1 rounded-full text-xs ${
                      getRevenueTier(sale.totalAmount) === "Tier 1" 
                        ? "bg-green-100 text-green-800" 
                        : getRevenueTier(sale.totalAmount) === "Tier 2"
                        ? "bg-blue-100 text-blue-800"
                        : getRevenueTier(sale.totalAmount) === "Tier 3"
              ...
              <tr>
                <td colSpan="8" className="py-4 text-center text-gray-500">
                  No sales statements found for this user
                </td>
              </tr>
            )}
          </tbody>
