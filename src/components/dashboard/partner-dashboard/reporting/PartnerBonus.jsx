...
export default function PartnerBonus({ onClose, partnerType }) {
  ...
  const generateDummyData = () => {
    ...
    quarters.forEach(q => {
      statuses.forEach((status, index) => {
        if (isSalesAgent) {
          dummyData.push({
            id: `sales-agent-bonus-${q}-${index}`,
            quarter: q,
            year: year,
            amount: (800 + (Math.random() * 1200)).toFixed(2),
            paymentStatus: status,
            dealsClosed: Math.floor(5 + (Math.random() * 15))
          });
        } else {
          dummyData.push({
            id: `partner-mw-bonus-${q}-${index}`,
            quarter: q,
            year: year,
            amount: (2000 + (Math.random() * 5000)).toFixed(2),
            paymentStatus: status,
            mwGenerated: (25 + (Math.random() * 100)).toFixed(1)
          });
        }
      });
    });
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
      <div className="bg-white w-full max-w-5xl rounded-md shadow-lg p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <h2 className={`${styles.pageTitle} text-left mb-6`}>
          {isSalesAgent ? "Partner Flat Bonus" : "Partner MW Bonus"}
        </h2>
        ...
        <div className="mb-6">
          ...
          >
            {loading ? (
              ...
              `Search ${isSalesAgent ? 'Partner Flat' : 'Partner MW'} Bonus`
            )}
          </button>
        </div>
            ...
            <table className="w-full table-auto border-collapse">
              <thead>
                <tr className="text-left text-[#1E1E1E] bg-gray-100">
                  ...
                  {isSalesAgent ? (
                    <th className="py-3 px-4 font-sfpro font-semibold border-b border-gray-200">Deals Closed</th>
                  ) : (
              ...
              <tbody>
                {bonusData.map((bonus, index) => (
                  <tr key={bonus.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50 hover:bg-gray-100"}>
                    ...
                    <td className="py-3 px-4 font-sfpro border-b border-gray-200">
                      {isSalesAgent ? `${bonus.dealsClosed} Deals` : `${bonus.mwGenerated} MW`}
                    </td>
                    <td className="py-3 px-4 font-sfpro border-b border-gray-200">
        ...
        {bonusData.length === 0 && !loading && quarter && year && (
          <div className="text-center py-8">
            <p className="font-sfpro text-gray-500">No {isSalesAgent ? 'partner flat' : 'partner MW'} bonus data found for Q{quarter} {year}</p>
          </div>
        )}

        {!quarter || !year ? (
          <div className="text-center py-8">
            <p className="font-sfpro text-gray-500">Please select a quarter and year to search for bonuses</p>
          </div>
        ) : null}
      </div>
    </div>
