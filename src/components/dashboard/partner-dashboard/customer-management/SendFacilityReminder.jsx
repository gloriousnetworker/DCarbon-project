...
const SendReminderModal = ({ isOpen, onClose, customerEmail }) => {
  ...
  const handleSendReminder = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        `/api/user/operator-reminders/${userId}`,
        body,
        {
          headers: {
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="relative bg-white rounded-lg shadow-lg w-full max-w-md mx-4 p-4 md:p-6 max-h-[90vh] overflow-y-auto max-h-[90vh] overflow-y-auto">
        <button
          onClick={onClose}
