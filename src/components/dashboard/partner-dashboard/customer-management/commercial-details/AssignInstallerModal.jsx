...
export default function EditResidentialFacilityModal({ facility, customerEmail, onClose = () => {}, onSave = () => {}, isOpen = false }) {
  ...
  const fetchPartnerDetails = async () => {
    ...
    setLoading(true);
    try {
      const response = await axiosInstance.get(`/api/user/partner/user/${userId}`, {
  ...
  const fetchUserInstallers = async () => {
    ...
    try {
      const response = await axiosInstance.get(`/api/user/get-users-referrals/${userId}`, {
        headers: { Authorization: `Bearer ${authToken}` }
      });
      if (response.data.status === "success") {
  ...
  const assignInstallerToReferral = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.put(`/api/user/referral/assign-installer/${userId}`, payload, {
        headers: { Authorization: `Bearer ${authToken}` }
      });
  ...
  const updateFacilityInstaller = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.put(`/api/residential-facility/update-facility/${facility.id}`, updateData, {
        headers: { Authorization: `Bearer ${authToken}` }
      });
